...
- 표지·펼침·섞기 카드 모두 **동일한 크기와 텍스트 스케일**로 통일하여 한 화면 안에서 일관된 카드 디자인을 유지합니다.
- 콤팩트한 토글 버튼 하나로 `카드 섞기`(24개 무작위) ↔ `다시 정렬`(계열 순서)을 오갈 수 있습니다.
...
### 4. 어른모드 / 어린이모드
- 상단의 콤팩트한 토글로 카드에 표시되는 감정 낱말을 **어른모드(`word`)** 와 **어린이모드(`childWord`)** 로 전환할 수 있습니다.
...
│        │  └─ not-found.tsx
│        ├─ components/
│        │  ├─ Header.tsx           # 앱 타이틀 + 콤팩트 토글 모음
│        │  ├─ SegmentedControl.tsx # 앱 공통 알약형 토글 (compact 옵션)
│        │  ├─ DisplayModeToggle.tsx# 어린이/어른모드 토글
│        │  ├─ ModeToggle.tsx       # 전체 보기/랜덤뽑기 토글
│        │  ├─ NormalMode.tsx       # 전체 보기 모드 (섞기/정렬 단일 토글)
│        │  ├─ RandomMode.tsx       # 랜덤뽑기 모드
│        │  ├─ FamilyCard.tsx       # 계열 카드(펼침 토글)
│        │  ├─ EmotionCard.tsx      # 개별 감정 카드 (compact 통일 레이아웃)
│        │  ├─ CardBack.tsx         # 카드 뒷면
│        │  ├─ FilterToggle.tsx     # 4/8/24 범위 선택
│        │  ├─ Footer.tsx           # 제작자/이론 출처
...
- 상단 토글은 공통 `SegmentedControl`을 콤팩트 모드로 사용해 공간을 적게 차지하며, 좁은 화면에서는 자연스럽게 두 줄로 줄바꿈됩니다.
